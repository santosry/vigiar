...
- `inst/extdata/rj_municipios_ibge_reference.csv` locks the 92 RJ
  municipality names and 7-digit codes from the IBGE Localidades API.
...
absent municipalities, records temporal gaps, flags possible Power BI
...
truncated, lacks a municipality code, lacks a schema hash, or otherwise
fails. This is intentionally stricter than checking that
`nrow(dados) > 0`.

The objective conclusion is one of `complete`, `partial`, `truncated`,
`schema_changed`, or `failed`. `complete` means that the expected RJ
grid was observed for the audited table and time grain. It does not
prove causal validity, exposure measurement validity, model adequacy, or
epidemiological identifiability.
